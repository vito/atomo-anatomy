...
function bindKeys() {
    $(".slides").keydown(function(e){
        ...
        console.log(detail[position], $(".group-" + position).length);
        var d = detail[position],
            group = $(".group-" + position),
            slides = $(".slide:not(.continue)");

        if (e.keyCode == 39 && position + 1 < slides.length)
            $(".slide").each(moveX(-1));
        else if (e.keyCode == 37 && position > 0)
            $(".slide").each(moveX(1));
        else if (e.keyCode == 40 && d + 1 < group.length)
            $(".group-" + position).each(moveY(-1));
        else if (e.keyCode == 38 && d > 0)
            $(".group-" + position).each(moveY(1));
    });
}
